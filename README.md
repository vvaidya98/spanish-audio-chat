...
✅ Grammar Breakdown Icons — a ⓘ next to each sentence (in Display Spanish and the Transcript) shows a short note on how that sentence's structure differs from English (phrase, literal translation, natural English syntax, a one-line pattern observation); generated by Claude in the background right after a story loads or regenerates, so the icon starts grayed/disabled and becomes clickable a few seconds later without ever delaying Play  
