...
- **Real pre-existing issue discovered, flagged not fixed:** `index.html` has loaded Tailwind from `cdn.tailwindcss.com` since the project's literal first commit — redundant next to the real PostCSS build, functionally harmless, but a `console.warn` on every page load that's silently evaded every "zero console errors" check this whole session (filtered for `error`, not `warn`). See SAC-087 below (originally logged as SAC-084, since renumbered — see item 5's own note for why).
- Version bumped to v1.2d.

### Shipped in v1.2e — Prompt #040, SAC-085 (Grammar Loading State + Box Emojis + Inline Scenario Emojis)
- **ID collision caught in this project's own backlog:** this round's prompt claimed "SAC-085," already assigned (during SAC-083) to an unbuilt caching idea — renumbered that idea to SAC-086 rather than let two things share one ID (see item 4 above).
- **Grammar loading state:** the box now appears the instant Grammar is checked, showing "💡 Loading explanation…" (new `ExplanationLoading` component, same shape/color as the real panel) until the background fetch resolves — SAC-083 fixed the interaction, this fixes the missing feedback while waiting. Scoped to Display Spanish only; the Transcript's ⓘ icon stays disabled-until-ready.
- **Box emojis:** 🇪🇸/🇬🇧/💡 now shown inside each box itself, not just on the checkbox labels.
- **Inline scenario emoji — verified before touching anything:** checked the actual story header first (`getBoundingClientRect`/screenshot) and confirmed it was already correctly inline from SAC-083 — no change needed there. The real "own line" emoji was on the picker cards in `ScenarioSelector.jsx` (a `block`-display span above the title) — fixed there instead, plus the "Create Custom Topic" card for grid-wide consistency.
- **Verified live:** loading placeholder appears immediately, swaps cleanly to real content (~13s); box emojis correct; scenario cards render as one inline line; zero console errors.
- Version bumped to v1.2e.

### Shipped in v1.2f — Prompt #041, SAC-084 (Clarity Mode Visibility)
- **Two genuine ambiguities resolved before building** (asked directly, since these were new-surface architecture questions, not fillable polish detail): (1) "SSML verification" logging doesn't map to anything real — this app has never used SSML, Clarity's pauses are plain `setTimeout(pauseMs)` delays; logs the real mechanism instead. (2) Indicator placement — resolved as always inside the Spanish text box, visible whenever Spanish text is shown and Clarity isn't Off.
- **New indicator row** inside the blue Spanish box: one mark per pause actually inserted so far in the current sentence — grey dot (Low) → yellow dot (Medium) → orange dash (High) → red double-dash (Ultra) — plus "y" always showing a red dot regardless of level (a far more frequent, comma-like pause than the other connector words), plus a "⏱️ Xms" elapsed-time readout. Reset fresh every sentence; correctly present-but-empty for sentences with no connector words.
- **`splitByConnectors()` extended** to also return which connector ended each segment, needed to distinguish "y" from the other five. Console logs the real pause mechanism at each checkpoint.
- **All three Display checkboxes now default unchecked**, including Grammar (reversing SAC-081's deliberate on-default — no longer needed now that SAC-085's loading placeholder gives instant feedback regardless of default state).
...
### Shipped in v1.2i — Prompts #047/#048, SAC-087 (Vocabulary Preview, Hybrid Loading)
- **Critical question answered:** `story.vocabularyByIndex[currentIndex]` doesn't exist — no per-sentence, difficulty-rated vocabulary structure existed anywhere before this round. New `/api/generate-vocabulary-preview` endpoint built to provide it (0-5 moderate/hard words per sentence, skips easy/common words entirely).
- **New checkbox "📖 Vocabulary Preview"**, listed first (before Spanish/English/Grammar), coral (`bg-secondary-light`) block.
- **Hybrid two-phase loading:** sentence 0's data (Grammar + Vocabulary Preview) fetched immediately on story load (~2.7s, ready before Play); sentences 1-9 fetched together once Play is actually pressed. Also applied to the pre-existing Grammar explanations fetch, replacing SAC-079's original all-at-once-on-load approach.
...
4. SAC-086 (idea, not yet built; renumbered *twice* now — originally logged as "SAC-080" during the SAC-079 round, moved to "SAC-085" once SAC-080 got independently claimed by the real Screen Wake Lock feature (shipped v1.2b), then moved again to SAC-086 once *that* number also got independently claimed by the real SAC-085 prompt (Grammar loading state + emojis, shipped v1.2e) — same resolution pattern as the SAC-014-E/F/G and roadmap-doc collisions much earlier in this project, just recurring more often lately as the backlog and the live prompt numbering both grow): cache `/api/generate-sentence-explanations` responses, same `(scenario, difficulty)`-keyed shape as `story_cache` — SAC-079 shipped this deliberately uncached, so a warmed Beginner scenario replayed by many different users currently re-generates the same explanations from scratch every time, a real ongoing cost that caching would eliminate for the 8 pre-built scenarios (custom topics are one-off text anyway, so wouldn't benefit the same way).
5. SAC-088 (idea, not yet built; originally logged as "SAC-084" during the SAC-083 round, renumbered to "SAC-087" once that number got independently claimed by the real Clarity Mode Visibility prompt (shipped v1.2f), now renumbered again to SAC-088 once *that* number also got independently claimed by the real SAC-087 prompt (Vocabulary Preview, Prompt #047) — the fifth ID collision in a row between this backlog's own bookkeeping and live incoming prompts, following SAC-080→085→086 and SAC-084→087→088 — worth being extra careful checking PENDING.md before assigning any new SAC number, in either direction): remove the redundant `<script src="https://cdn.tailwindcss.com">` from `index.html` — present since the project's first commit, functionally harmless alongside the real PostCSS-compiled Tailwind build, but adds an external dependency, extra page weight, and a `console.warn` on every load that's evaded every "zero console errors" check made anywhere in this project's history (see CLAUDE.md Known Issues #5). Should be a safe, isolated removal, but warrants its own go-ahead and a visual regression pass rather than being bundled into an unrelated round.
