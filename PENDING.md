...
- **Verified live** (real Chrome against the actual dev servers, uncached scenarios): first load (scenario click → Play ready) dropped from the reported 40s+ to ~21s — matching almost exactly the direct-measured cost of the `/api/generate-story` call alone (a raw `curl` to `/api/story-questions` independently measured ~21s too, confirming ~21s + ~21s ≈ the originally-reported ~40s, and that the fix targets the real bottleneck). Play click → confirmed playing: ~0.04s (was previously bottlenecked behind the same blocked wait). Comprehension Check populates correctly once the background fetch resolves, including the edge case of manually jumping to the story's end before that fetch finishes (no crash, button appears retroactively once data arrives). One test artifact worth noting for future rounds: a first attempt showed the background fetch not resolving within 60s — turned out to be React StrictMode's dev-only double-invoke doubling up concurrent Claude calls (same well-documented inflation pattern as the v1.0m round), not a regression; a direct `curl` confirmed the endpoint's real single-call latency (~21s).
